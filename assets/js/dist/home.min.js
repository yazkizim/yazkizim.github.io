...
if ($('#sozlukid').length > 0) {
	$.ajax({
		...
		success: function(data) {
			...
			var b = document.getElementById('sozlukid'); //or grab it by tagname etc
			b.href = "/sozluk/?kelime=" + data[a].kelime;
...
var url = 'https://corsproxy.io/?' + encodeURIComponent('https://api.genelpara.com/embed/doviz.json');
$.ajax({
	url: url,
	dataType: 'json',
	async: false,
	success: function(data) {
		...
		if (data['btc']['d_oran'] > 0) { 
			$('#bist').attr('class', 'text-success');
			$('#bist').html(parseFloat(data['BTC']['satis']).toFixed(2) + ' ₺');
		} else {
			$('#bist').attr('class', 'text-danger');
			$('#bist').html(parseFloat(data['BTC']['satis']).toFixed(2) + ' ₺');
		}
